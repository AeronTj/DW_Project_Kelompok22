...
def get_sql_connection():
    """Create SQL Server connection with sensible defaults for host and password.

    - Prefer env vars `MSSQL_HOST` or `DB_SERVER` for host.
    - Prefer env vars `MSSQL_SA_PASSWORD` or `SA_PASSWORD` for password.
    - If running inside Docker (/.dockerenv present), default host to 'sqlserver'.
      Otherwise, default to 'localhost'.
    """
    try:
        # Determine default host based on runtime context
        default_host = 'sqlserver' if os.path.exists('/.dockerenv') or os.getenv('AIRFLOW_HOME') else 'localhost'
        ...
        logging.info(f"Connecting to SQL Server at {db_server}...")
